...
PacketBuffer.prototype.write = function write(data) {
  ...
  while (data.length > 0) {
    ...
    if (this.buffer != null) {
      ...
      if (this.buffer.length < 2) {
        var old = this.buffer[0];        // save old byte
        var size = (data[0] << 8) + old; // convert from little-endian
        this.buffer = new Buffer(size);  // make new buffer
...
PacketBuffer.prototype.read = function() {
